...
class TFTrainer:
    ...
    def get_eval_train_tfdataset(self, eval_train_dataset: Optional[tf.data.Dataset] = None) -> tf.data.Dataset:
        if eval_train_dataset is None and self.eval_train_dataset is None:
            raise ValueError("Trainer: evaluation requires an eval_dataset.")

        eval_train_dataset = eval_train_dataset if eval_train_dataset is not None else self.eval_train_dataset
        ds = (
            eval_train_dataset.cache()
    ...
    def _setup_wandb(self):
        ...
        wandb.init(project=os.getenv("WANDB_PROJECT", "TwinBertGNN"), config=vars(self.args))
    ...
    def _prediction_train_loop(
        ...
        metrics["eval_train_loss"] = loss.numpy()

        for key in list(metrics.keys()):
            if not key.startswith("eval_train_"):
                metrics[f"eval_train_{key}"] = metrics.pop(key)
    ...
    def evaluate_train(
        self, eval_train_dataset: Optional[tf.data.Dataset] = None, prediction_loss_only: Optional[bool] = None
    ) -> Dict[str, float]:
        """
        Prediction/evaluation loop, shared by `evaluate()` and `predict()`.
        """
        eval_train_ds = self.get_eval_train_tfdataset(eval_train_dataset)

        output = self._prediction_train_loop(eval_train_ds, description="Evaluation Train")
    ...
    def train(self, finetune=False, previous_epoch=10, do_eval_train=False) -> None:
        """
        ...
        for epoch_iter in range(start_epoch, int(epochs + 1)):
            ...
            for step, training_loss in enumerate(self._training_steps(train_ds, optimizer)):
                ...
                self.epoch_logging = epoch_iter - 1 + (self.global_step % self.train_steps + 1) / self.train_steps
                ...
                if self.global_step % 1000 == 0:
                    ...
                    logger.info('Epoch: %d; Step: %d; Current Epoch Elapse/Estimate: %0.2fs/%0.2fs' % (epoch_iter, step + 1, time_elapse, time_est))
